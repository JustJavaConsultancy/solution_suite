...
public class RuleEngineRestController {
    ...
    public ResponseEntity<?> createRule(@RequestBody Rule rule) {
        rule = knowledgeBaseService.saveRule(rule);
    ...
    @GetMapping(value = "/get-all-org-rules/{orgcode}")
    public ResponseEntity<?> getRulesByOrganization(@PathVariable("orgcode") String orgcode) {
           List<Rule> allOrgRules = knowledgeBaseService.getAllRuleByOrg(orgcode);
        return ResponseEntity.ok(allOrgRules);
    }
